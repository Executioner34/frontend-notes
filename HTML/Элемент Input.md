...
`type` | `button` — создает кнопку. <input type="button" value="Кнопка"><br>`checkbox` — превращает поле ввода во флажок, который можно установить или очистить, например, У меня есть автомобиль
