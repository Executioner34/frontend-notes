...
`type` | `button` — создает кнопку. <input type="button" value="Кнопка"><br>`checkbox` — превращает поле ввода во флажок, который можно установить или очистить, например, <input type="checkbox">У меня есть автомобиль

color — генерирует палитры цветов в поддерживающих браузерах, давая пользователям возможность выбирать значения цветов в шестнадцатеричном формате. <input type="color">
